...
public abstract class AbstractCallback<T> implements Callback {
    ...
    public void onResponse(Call call, Response response) throws IOException {
        if (response.code() == 200) {
            try {
                ...
                final T t = parseResponse(call, body);
                successData(t);
                // check error status
                checkErrorStatus(t, call);
            } catch (Exception e) {
    ...
    private void checkErrorStatus(T t, Call call) {
        if (t instanceof Result) {
            int resultCode = ((Result) t).getResultCode();
            if (resultCode == Result.CODE_TOKEN_ERROR) {
                Request req = call.request();
               // 接口返回 登录token过期，记录到日志中
                com.sk.weichat.util.log.LogUtils.e("STATUS_USER_TOKEN_OVERDUE", "request:" + req.toString() + "request body: "+ req.body());
            }
        }
    }
