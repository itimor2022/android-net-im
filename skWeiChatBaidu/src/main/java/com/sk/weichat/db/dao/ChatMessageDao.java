...
public class ChatMessageDao {
     ...
     * 删除聊天记录中某人发送的所有消息
     */
    public boolean deleteAllChatMessageBySender(String ownerId, String friendId , String fromUserId /*消息发送方的ID*/) {
        Log.d("deleteHis", ownerId+"->"+friendId+"->"+fromUserId);
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return false;
        }
        try {
            List<ChatMessage> chatMessages = dao.queryForEq("fromUserId", fromUserId);
            if (chatMessages != null && chatMessages.size() > 0) {
                dao.delete(chatMessages);
                return true;
            }
    ...
    public ChatMessage getLastChatMessage(String ownerId, String friendId, long time) {
        ...
        try {
            QueryBuilder<ChatMessage, Integer> builder = dao.queryBuilder();
            builder.where().le("timeSend", time * 1000);
            builder.orderBy("timeSend", false);
            chatMessage = dao.queryForFirst(builder.prepare());
        } catch (SQLException e) {
            e.printStackTrace();
