 ...
 * @Describe 激活会员
 */
public class ActivationActivity extends BaseAuthActivity implements View.OnClickListener {
    ...
    private void activationVip(){
        ...
        if(TextUtils.isEmpty(account) || account.length() != 16){
            dimessDialog();
            ToastUtil.showToast(ActivationActivity.this, "请输入16位数编码");
            return;
        }
        ...
        if(passwordType == 0){
            if(TextUtils.isEmpty(password) || password.length() < 6 || password.length() > 20){
                dimessDialog();
                ToastUtil.showToast(ActivationActivity.this, "请输入正确格式密码");
                return;
            }
...
//                            MainActivity.isActivated = true;
                            ToastUtil.showToast(ActivationActivity.this, "激活成功");
                            EventBus.getDefault().post(new ActivationEvent());
                            ActivationActivity.this.finish();
                        }else{
                            ToastUtil.showToast(ActivationActivity.this, result.getMsg());
                        }
                    }
                    ...
                    public void accept(Throwable throwable) throws Throwable {
                        dimessDialog();
                        ToastUtil.showNetError(ActivationActivity.this);
                    }
                });
    }
