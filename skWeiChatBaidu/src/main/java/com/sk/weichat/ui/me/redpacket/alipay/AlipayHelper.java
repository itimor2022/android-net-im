...
public class AlipayHelper {
    ...
    private static void callAlipay(Activity activity, String orderInfo) {
        ...
        AsyncUtils.doAsync(activity, new AsyncUtils.Function<Throwable>() {
            @Override
            public void apply(Throwable t) throws Exception {
                Reporter.post("拉起支付宝失败，", t);
                AsyncUtils.runOnUiThread(activity, ctx -> {
                    ToastUtil.showToast(ctx, R.string.tip_alipay_failed);
                });
            }
        }, new AsyncUtils.Function<AsyncUtils.AsyncContext<Activity>>() {
            @Override
            public void apply(AsyncUtils.AsyncContext<Activity> c) throws Exception {
                ...
                c.uiThread(new AsyncUtils.Function<Activity>() {
                    @Override
                    public void apply(Activity r) throws Exception {
