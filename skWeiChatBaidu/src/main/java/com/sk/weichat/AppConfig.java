...
public class AppConfig {
    ...
    // 请求地址
    public static final String APP_URL = "http://im-api.l93lnt5oowndudkio7yw79yv8zj53onk.com/";
    public static String CONFIG_URL = "http://im-api.l93lnt5oowndudkio7yw79yv8zj53onk.com/config";// Socket 咕喃 服务器入口
...
//图片下载地址
    ...
    public static String[] HOST_LIST = {
            "http://im-api.l93lnt5oowndudkio7yw79yv8zj53onk.com/config",
            "http://im-api.l93lnt5oowndudkio7yw79yv8zj53onk.com/config"
    };

    // 发现 - 证书注册
    public static String CERTFICATE_WEB_URL = "https://check.cert.shijiezhixin.com/#/page/index";

    // 发现 - 小家田园、金银山
    public static String Home_Farm_WEB_URL = "https://home.farm.ssrsgroup.com/";
    ...
    public String signin;  //用户签到
    public String isSingin;  //查询当天是否已签到
    ...
    public int uploadVersion;// 是否强制更新
    ...
    public String ROOM_MEMBER_BATCH_DEL;// 删除成员 || 退出群组 支持批量
    public String ROOM_TRANSFER;// 转让群组
    public String ROOM_COPY;//群复制

    public String SENSITIVE_CHAT_WORDS; //聊天敏感词
    ...
    public String FIND_MORE_ITEMS;// 发现
    ...
    public String USDT_TRANSFER; // usdt提现
    ...
    public String MANUAL_PAY_USDT_RECHARGE; // usdt充值
    /* 组织架构 */
    ...
    // 新增的充值提现配置
    public boolean enableUsdtPay;
    public boolean enableWxWithdrawal;
    public boolean enableAliWithdrawal;
    public boolean enableBankWithdrawal;
    public boolean enableUsdtWithdrawal;
    ...
    public int isOpenUI;
    public int showGroupManagerAction;
    public boolean researchSwitch;
    public int withdrawStyleSwitch = 0; //显示提现方式开关 0-手动提现 1-自动提现
    public String withdrawRate; // 提现费率
    public Double usdtExchangeRate; // usdt汇率
    public int payStyleSwitch; // 充值方式开关 0-手动扫码充值 1-自动充值
    public String macAppUrl;
    public String macExplain;

    public ScanRecharge rechargeusdtInfo; // 平台usdt相关账户信息
    ...
    public static boolean isShiku() {
        return !TextUtils.isEmpty(apiKey)
                ...
                && Objects.equals(BuildConfig.APPLICATION_ID, "com.gunan.im");
    }

    public static boolean showMessageBack = false;//显示撤回消息的提示
    public static boolean closeVideoMeeting = true;//关闭视频会议
    ...
    public static void saveConfigUrl(Context ctx, String str) {
        ...
        /*Roken end 20220519*/
        PreferenceUtils.putString(ctx, "APP_SERVICE_CONFIG", str);
    }
    ...
    public static AppConfig initConfig(ConfigBean configBean) {
        return initConfig(configBean, null);
    }

    /**
     * 会改变的配置
     **/
    public static AppConfig initConfig(ConfigBean configBean, AppConfig cf) {
        ...
        promotionUrl = configBean.getPromotionUrl();
        downloadAppUrl = configBean.getWebsite();
        /* 1、Api 的服务器地址 */
        config.apiUrl = configBean.getPipeInfo(configBean.getApiUrl(), configBean.getApiUrl2());

        /* 2、上传的服务器地址 */
        config.uploadUrl = configBean.getUploadUrl();

        config.uploadVersion = configBean.getUploadVersion();

        /* 3、头像下载地址 */
        config.downloadAvatarUrl = configBean.getDownloadAvatarUrl();
        config.downloadUrl = configBean.getPipeInfo(configBean.getDownloadUrl(), configBean.getDownloadUrl2());
...
//        config.ordinaryUserCannotCreateGroup = configBean.getIsCommonCreateGroup() > 0;
        config.ordinaryUserCannotCreateGroup = false;
        ...
        config.displayRedPacket = configBean.getDisplayRedPacket() == 0;//是否禁用红包相关功能 true表示禁用，

        config.disableLocationServer = configBean.getIsOpenPositionService() > 0;//开启位置服务
        ...
        config.enableUsdtPay = configBean.getEnableUsdtPay() == 1;
        config.enableWxWithdrawal = configBean.getEnableWxWithdrawal() == 1;
        config.enableAliWithdrawal = configBean.getEnableAliWithdrawal() == 1;
        config.enableBankWithdrawal = configBean.getEnableBankWithdrawal() == 1;
        config.enableUsdtWithdrawal = configBean.getEnableUsdtWithdrawal() == 1;
        ...
        config.XMPPHost = configBean.getPipeInfo(configBean.getXMPPHost(), configBean.getXMPPHost2());

        config.XMPPDomain = configBean.getPipeInfo(configBean.getXMPPDomain(), configBean.getXMPPDomain2());

        config.xmppPingTime = configBean.getXmppPingTime();

        config.XMPPTimeOut = configBean.getXMPPTimeout();

        config.JitsiServer = configBean.getPipeInfo(configBean.getJitsiServer(), configBean.getJitsiServer2());
        config.isOpenUI = configBean.getIsOpenUI();
        config.showGroupManagerAction = configBean.getShowGroupManagerAction();
        config.researchSwitch = configBean.getResearchSwitch() == 1;

        config.macAppUrl = configBean.getMacAppUrl();
        config.macExplain = configBean.getMacExplain();
...
//        String apiUrl = "http://192.168.254.40:8092/";
        //签到
        config.signin = apiUrl + "user/signin";  //
        config.isSingin = apiUrl + "user/isSignin";
        // 一键添加群成员权限
        config.addGroup = apiUrl + "user/addGroupAllMan";
        //参数：roomId  房间id

        //查询是否有一键添加群成员权限
        config.isAddGroup = apiUrl + "user/isAddGroupAllMan";
        //无参数
        ...
        config.USER_ONE_CLICK_LOGIN = apiUrl + "user/oneClickLogin";// 一键登录
        ...
        config.FIND_MORE_ITEMS = apiUrl + "general/friendsterWebsiteList";// 发现
        ...
        config.ROOM_MEMBER_BATCH_DEL = apiUrl + "room/member/deleteBatch"; // 删除成员 || 退出群组(批量)
        ...
        /* 聊天敏感词*/
        config.SENSITIVE_CHAT_WORDS = apiUrl + "sensitiveWords"; // 获取聊天敏感词
        /* 商务圈 */
        ...
        config.USDT_TRANSFER = apiUrl + "usdt/withdraw"; // usdt提现
        ...
        config.MANUAL_PAY_USDT_RECHARGE = apiUrl + "usdt/recharge"; // usdt充值余额
