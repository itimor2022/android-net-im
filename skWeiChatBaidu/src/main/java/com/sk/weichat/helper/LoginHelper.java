        ...
        if (!idIsEmpty && UserSp.getInstance(context).isLogged()) {// 用户标识都不为空，那么就能代表一个完整的用户
            ...
            if (!LoginHelper.isUserValidation(user)) {// 用户数据错误,那么就认为是一个游客
                ...
                if (LoginHelper.isTokenValidation()) {// Token未过期
                    ...
                    // 日志记录
                    com.sk.weichat.util.log.LogUtils.e("STATUS_USER_TOKEN_OVERDUE", "idIsEmpty: " + idIsEmpty
                            + "isLogged: " + UserSp.getInstance(context).isLogged()
                            + "user: " + user.toString()
                            + "accessToken: " + UserSp.getInstance(MyApplication.getContext()).getAccessToken()
                            + "isUpdate: " + UserSp.getInstance(context).isUpdate(true)
                            );
                }
            }
