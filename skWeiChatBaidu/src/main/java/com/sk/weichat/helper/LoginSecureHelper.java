...
public class LoginSecureHelper {
    ...
    private static void generateBeforeLoginParam(Context ctx, Map<String, String> params) {
        try {
            ...
            String mac = MAC.encodeBase64(macContent.getBytes("utf-8"), key);
            params.put("salt", salt);
            params.put("secret", mac);
        }catch (Exception e){
                                        ...
                                        if (Result.checkSuccess(ctx, result, false) && result.getData() != null && result.getData().getData() != null) {
                                            ...
                                            c.uiThread(r -> {
                                                if (Result.checkError(result, Result.CODE_LOGIN_TOKEN_INVALID)) {
                                                    ...
                                                    com.sk.weichat.util.log.LogUtils.e("STATUS_USER_TOKEN_OVERDUE", "errorCode:"+Result.CODE_LOGIN_TOKEN_INVALID + "-->" +
                                                            "url: " + coreManager.getConfig().USER_LOGIN_AUTO +
                                                            "params:  salt:" + salt + " loginToken:" + loginToken
                                                            + " data:" + data
                                                            );
                                                } else {
    ...
    public static String decodeLoginResult(byte[] code, String data) {
        try {
