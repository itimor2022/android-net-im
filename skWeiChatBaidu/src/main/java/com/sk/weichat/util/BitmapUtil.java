...
public class BitmapUtil {
    ...
    public static Bitmap getBitmap(Bitmap b, int x, int y, int w, int h) {
        Bitmap bitmap = b;
        try {
            int bitmapHeight = h - (y < 0 ? 10 : y);
            bitmapHeight = bitmapHeight > bitmap.getHeight() ? bitmap.getHeight() : bitmapHeight;
            bitmap = (Bitmap) Bitmap.createBitmap(bitmap, x < 0 ? 0 : x, y < 0 ? 0 : y, w - (x < 0 ? 10 : x), bitmapHeight);
