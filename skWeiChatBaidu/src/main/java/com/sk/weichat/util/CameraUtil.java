...
public final class CameraUtil {
    ...
    public static Uri getOutputMediaFileUri(Context context, int type, boolean checkProvider) {
        File file = getOutputMediaFile(context, type);
        if (file == null) {
            return null;
        }
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                for (ResolveInfo resolveInfo: resolveInfos) {
                    ...
                    activity.grantUriPermission(pkgName, outputUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
                }
            }
    ...
    public static void cropImage(Activity activity, Uri originalFileUri, Uri outputFileUri, int requestCode, int aspectX, int aspectY, int outputX,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            for (ResolveInfo resolveInfo: resolveInfos) {
                String pkgName = resolveInfo.activityInfo.packageName;
                activity.grantUriPermission(pkgName, originalFileUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
                activity.grantUriPermission(pkgName, outputFileUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
            }
        }
        activity.startActivityForResult(intent, requestCode);
    }
