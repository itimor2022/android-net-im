...
public class XMuChatMessageListener {
    ...
    public void onReceMessage(com.sk.weichat.socket.msg.ChatMessage message, ChatMessage chatMessage, boolean isDelay) {
        String roomJid;
...
*/
            // todo 以上为xmpp的判断逻辑，因为xmpp无法精准的返回群组未读消息条数，只是返回最后100条离线消息，而当前socket虽然也只是返回最后100条离线消息，
            //  但是还返回了群组未读消息的准确条数，所以当 未读消息条数>实际返回的消息数量时，我们对当前群组漫游任务endTime进行赋值，反之删除，
            //  具体逻辑处理可至EMConnectionManager类tag为batchJoinRoom的Log日志下查看
        }
        ...
        if (type == XmppMessage.TYPE_BACK) {
            ...
            if (!AppConfig.showMessageBack) {
                tempLastChat = ChatMessageDao.getInstance().getLastChatMessage(mLoginUserId, roomJid);
            }
            ...
            if (AppConfig.showMessageBack) {
                ...
                if (tempLastChat != null) {
                    if (tempLastChat.getPacketId().equals(packetId)) {
                        // 要撤回的消息正是朋友表的最后一条消息
                        if (chatMessage.getFromUserId().equals(mLoginUserId)) {// 自己发的不用处理
                            FriendDao.getInstance().updateFriendContent(mLoginUserId, roomJid,
                                    chat == null ? "" : chat.getContent(), XmppMessage.TYPE_TEXT, chatMessage.getTimeSend());
                        } else {
                            FriendDao.getInstance().updateFriendContent(mLoginUserId, roomJid,
                                    chat == null ? "" : chat.getContent(), XmppMessage.TYPE_TEXT, chatMessage.getTimeSend());
                        }
