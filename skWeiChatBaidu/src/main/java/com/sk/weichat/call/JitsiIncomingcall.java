...
//                    }
    ...
    private void sendHangUpMessage() {
        ...
        if (mCallType == 1) {
            ...
            message.setType(XmppMessage.TYPE_CANCLE_VOICE_MEETING);
            message.setContent("语音会议");
        }else if(mCallType == 4){
            message.setType(XmppMessage.TYPE_CANCLE_VIDEO_MEETING);
            message.setContent("视频会议");
        }
        ...
        if(mCallType == 3 || mCallType == 4){
            ...
            message.setFilePath(roomid);
            message.setObjectId(objectId);
            if (ChatMessageDao.getInstance().saveNewSingleChatMessage(mLoginUserId, roomid, message)) {
                FriendDao.getInstance().updateFriendContent(coreManager.getSelf().getUserId(), roomid, message.getContent(), message.getType(), TimeUtils.sk_time_current_time());
                ListenerManager.getInstance().notifyNewMesssage(mLoginUserId, roomid, message, true);
            }
        }else {
            if (ChatMessageDao.getInstance().saveNewSingleChatMessage(mLoginUserId, call_toUser, message)) {
                ListenerManager.getInstance().notifyNewMesssage(mLoginUserId, call_toUser, message, false);
