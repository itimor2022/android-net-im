...
public class UserSp extends CommonSp {
    ...
    public String getAccessToken() {
        String token = getValue(KEY_ACCESS_TOKEN, (String) null);
        if (TextUtils.isEmpty(token)) {
            LogUtils.e("getAccessToken: ", token);
        }
        return token;
    }
    ...
    public void setAccessToken(String value) {
        ...
        if (TextUtils.isEmpty(value)) {
            LogUtils.e("setAccessToken", value);
        }
    }
    ...
    public String getAccessToken(String defaultValue) {
        String token = getValue(KEY_ACCESS_TOKEN, defaultValue);
        if (TextUtils.isEmpty(token)) {
            LogUtils.e("getAccessToken: ", token);
        }
        return token;
    ...
    public void saveAutoLoginResult(LoginAuto result) {
        ...
        if (TextUtils.isEmpty(result.getAccessToken())) {
            LogUtils.e("STATUS_USER_TOKEN_OVERDUE: saveAutoLoginResult: ", result.getAccessToken());
        }
    }

    /* 注销登录时，将其他数据清空，只保留UserId这一个 */
    public void clearUserInfo() {
        LogUtils.e("com.sk.weichat.sp.UserSp.clearUserInfo", "");
